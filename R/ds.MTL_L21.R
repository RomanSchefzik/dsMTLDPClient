...
#' \item{\code{ter} \cr A value (1, 2 or 3) specifying one out of three termination rules to determine whether the optimization converges. The first rule (\code{ter=1}) checks whether the current objective value is close enough to 0. The second rule (\code{ter=1}) considers the last two objective values and checks whether the decrement is close enough to 0. The third rule (\code{ter=1}) allows the optimization to be performed for a certain maximum number of iterations (\code{maxIter}).}
